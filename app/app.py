...
with col2:
    ...
    k3.metric(
        "Change",
        f"${delta:,.0f}",
        None if np.isnan(delta_pct) else f"{delta_pct:+.1f}%",
    )
...
ab = bq_df(
    f"SELECT ab_group, users, converters, conversion_rate "
    f"FROM `{PROJECT}.{DATASET}.ab_metrics`",
    label="ab_metrics",
)
if not ab.empty:
    ...
    if {"A", "B"}.issubset(set(ab["ab_group"])):
        ...
        uplift = (
            p_t - p_c if np.isfinite(p_c) and np.isfinite(p_t) else float("nan")
        )
        ...
        chi2_stat = (
            ((n_c - exp) ** 2) / exp + ((n_t - exp) ** 2) / exp
            if np.isfinite(exp)
            else float("nan")
        )
        srm_p = (
            1 - chi2.cdf(chi2_stat, df=1) if np.isfinite(chi2_stat) else float("nan")
        )
        ...
        se = (
            np.sqrt(p_pool * (1 - p_pool) * (1 / n_c + 1 / n_t))
            if 0 < p_pool < 1
            else float("nan")
        )
        ...
        c1.metric(
            "SRM p-value",
            f"{srm_p:.3f}" if np.isfinite(srm_p) else "—",
            help="> 0.05 ≈ allocation ok",
        )
        c2.metric(
            "p-value (2-prop z)",
            f"{p_val:.3f}" if np.isfinite(p_val) else "—",
            help="< 0.05 ≈ significant",
        )
        c3.metric(
            "95% CI (uplift)",
            (
...
try:
    ...
    if not ret.empty:
        ...
        # Use gradient if matplotlib is present; else plain table
        try:
            idx = pd.IndexSlice
            styler = (
                ret_pivot.style
            ...
            fmt = ret_pivot.applymap(lambda x: f"{x:.0%}")
...
if show_diag:
    ...
    if _DIAG:
        diag_df = pd.DataFrame(_DIAG)[["label", "rows", "ms"]]
        diag_df = diag_df.sort_values("ms", ascending=False)
...
st.caption(
    "Data source: BigQuery views built by dbt "
    "(ab_group, funnel_users, revenue_daily, retention_weekly, ab_metrics)."
)
